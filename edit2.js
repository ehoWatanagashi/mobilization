$(document).ready(function () {
            ...
            $.each(data, function (key, data) {
                if ((key >= filterFrom) && (key <= filterTo)) {
                    ...
                    /*Вывыодим информацию о лекции, добавляем кнопки редактирования и удаления.
                     Помещаем все лекции, которые проходят в один день, в созданный ранее div*/
                    $.each(data, function (index, value) {
                        $("div.day-block" + keymap[key]).append('<div class="lesson-block"><span class="lesson-time">' + value['time'] + ' - ' + value['time2'] + '</span>' +
                            '<div class="lesson-info">' +
                            '<a href="#" class="lesson-subject">' + value['name'] + '</a>' +
                    ...
                    $.each(data, function (index, value) {
                        if (classroom == value['classroom']) {
                            ...
                            /* Добавляем кнопки редактирования и удаления.
                             Помещаем все лекции, которые проходят в один день, в созданный div*/
                            $("div.day-blocks" + keymap[key]).append('<div class="lesson-block"><span class="lesson-time">' + value['time'] + ' - ' + value['time2'] + '</span>' +
                                '<div class="lesson-info">' +
                                '<a href="#" class="lesson-subject">' + value['name'] + '</a>' +
                                '<p class="lesson-school">' + value['school'] + '</p>' +
                                '<p class="lesson-teacher">' + value['teacher'] + '</p>' +
    ...
    $('body').on('click', "button[id^='edit-timetable']", function () {
        ...
        $.getJSON('timetable.json', function (data) {
            ...
            $("#edit-time-timetable1").val(data[obj][value2]['time']);
            $("#edit-time-timetable2").val(data[obj][value2]['time2']);
        });
        loadPopup(id);
    });
    ...
    $("#save-new-timetable").click(function () {
        ...
        var timeTimetable1 = String($("#time-timetable1").val());
        var timeTimetable2 = String($("#time-timetable2").val());
            ...
            if (data[dateTimetable] != undefined) {
                ...
                for (i = 0; i < lenkeys; i++) {
                    if ((classroomName == data[dateTimetable][i]["classroom"]) &&
                        (((timeTimetable1 >= data[dateTimetable][i]["time"]) && (timeTimetable1 < data[dateTimetable][i]["time2"])) ||
                        ((timeTimetable2 > data[dateTimetable][i]["time"]) && (timeTimetable2 <= data[dateTimetable][i]["time2"])) ||
                            ((timeTimetable1 <= data[dateTimetable][i]["time"]) && (timeTimetable2 >= data[dateTimetable][i]["time2"]))) ) {
                        setError("2");
                    }
                    ...
                    /*Проверка, есть ли уже у преподавателя другие лекции в это же время*/
                    for (k = 0; k < len; k++) {
        ...
        if (timeTimetable1 == "") {
            setError("5");
            err_text = "Укажите время начала лекции!";
        }
        if (timeTimetable2 == "") {
            setError("8");
            err_text = "Укажите время окончания лекции!";
        }
        if (schoolTimetable == "") {
            setError("6");
        }
        if (timeTimetable1 > timeTimetable2) {
            setError("9");
        }
        ...
        setTimeout(function () {
            if (error == 0) { // если ошибок нет то отправляем данные
                ...
                if (error == 9) {
                    err_text = "Лекция не может заканчиваться раньше, чем она начинается!";
                }
    ...
    $("#save-edited-timetable").click(function () {
        ...
        var timeTimetable1 = String($("#edit-time-timetable1").val());
        var timeTimetable2 = String($("#edit-time-timetable2").val());
        //var lectureTimetable = $("#lecture-timetable").val();
        var teacherTimetable = $("#teacher-timetable").val();
